...
(function (global, factory) {
    ...
    var DateTime = function(trigger, options) {
        ...
        switch (type) {
            ...
            case 'date-range': case 'month-range': {
                ...
                if (type == 'date-range') {
                    input.val(arrBegin.join('-') + ' \t到\t ' + arrEnd.join('-'));
                } else {
                    input.val(arrBegin.slice(0, 2).join('-') + ' \t到\t ' + arrEnd.slice(0, 2).join('-'));
                }
    ...
    DateTime.prototype.val = function() {
        ...
        switch (this.type) {
            ...
            case 'date-range': {
                input.val(selected[0].join('-') + ' \t到\t ' + selected[1].join('-'));
                break;
            }
            case 'month-range': {
                input.val(selected[0].slice(0, 2).join('-') + ' \t到\t ' + selected[1].slice(0, 2).join('-'));
                break;
            }
